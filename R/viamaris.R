...
#'
#' A wrapper function to run gINLAnd using SNP data in ADEGENET's genind format#' @param sampleXY -  a data frame with three columns: ID, X and Y (in decimal degrees)
#' @param extent.buffer (optional) number of degrees to extend raster around your samples, default = 0.5 (you may need to increase this to allow navigation around land masses that extend beyond the extent of your samples)
...
viamaris <- function (sampleXY, extent.buffer = NULL, resolution = NULL, EPSG = NULL) {
  ...
  # check for required packages and install if nec.
  depnds <- function(x){
    for( i in x ){
  ...
  depnds(c("sp", "rgdal", "rgeos", "raster", "gdistance", "maptools"))
  ...
  # Read in Shapefile
  shp <- readOGR("inst/extdata/mapfiles", "ne_10m_land")
  #system.file("extdata", "Mfsub500.stru", package="melfuR")
  ...
  if(any(sampleXY$X < 0) && any(sampleXY$X > 0)) {
    ...
    ras.extent <- as(extent(minX, maxX, minY, maxY), 'SpatialPolygons')
    cut.shp <- shp
